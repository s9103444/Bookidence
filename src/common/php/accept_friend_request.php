...
$authHeader = $_SERVER['HTTP_AUTHORIZATION'] ?? $_SERVER['REDIRECT_HTTP_AUTHORIZATION'] ?? '';
 $token = str_starts_with($authHeader, 'Bearer ') ? substr($authHeader, 7) : '';
   ...
   if ($token === '') {
    ...
    $stmt=$pdo->prepare("
    UPDATE friendship SET rel_status='已成為好友'
    WHERE user_id_a=:fromUserId AND user_id_b=:myId AND rel_status='申請中'
    ");

    $stmt->execute(['fromUserId'=>$fromUserId,'myId'=>$member['user_id']]);
